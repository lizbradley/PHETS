...
def prep_save_worms_double():
	...
	ans = 'y'
	if ans == 'y':
		dir_a = 'output/DCE/trajectories/double/a/'
		for f in os.listdir(dir_a):
			os.remove(dir_a + f)

		dir_b = 'output/DCE/trajectories/double/b/'
		for f in os.listdir(dir_b):
			os.remove(dir_b + f)

		open('output/DCE/trajectories/double/a/WORM_INFO.txt', 'w').close()
		open('output/DCE/trajectories/double/b/WORM_INFO.txt', 'w').close()
	else:
		sys.exit()
...
def save_worms_double(filename1, filename2, i, tau1, tau2, crop_1, crop_2):
	...
	info_file_1 = open('output/DCE/trajectories/double/b/WORM_INFO.txt', 'a')
...
def prep_save_worms_single():
	...
	ans = 'y'
	if ans == 'y':
		dir = 'output/DCE/trajectories/single/'
		for f in os.listdir(dir):
			os.remove(dir + f)

		open('output/DCE/trajectories/single/WORM_INFO.txt', 'w').close()
	else:
		sys.exit()
...
def frames_to_movie(out_file_name, framerate=2):
	...
	in_str = ('ffmpeg -loglevel panic -y -framerate %i ' % framerate) + '-i DCE/frames/frame%03d.png'
