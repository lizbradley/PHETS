...
def plot_dists_to_means(
	...
	----------
	traj1: Trajectory
		Should have been initialized with ``num_windows`` parameter or have had
		its ``slice`` method called
	traj2: Trajectory
		Should have been initialized with ``num_windows`` parameter or have had
		its ``slice`` method called
	out_filename: str
	...
	weight_func: lambda
		Weight function applied to all prfs before computing distances\n
		default : None
	load_filts : bool or 2-tuple of str
		...
		default: ``False``
	save_filts : bool or 2-tuple of str
