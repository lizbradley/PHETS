...
def make_frames_2D_old(filt_data, title_block_info, color_scheme, alpha, frame_debug):
	def plot_witnesses(subplot, attractor_data):
		...
		x = attractor_data[:, 0]
		y = attractor_data[:, 1]
		return subplot.scatter(x, y, color='black', marker=matplotlib.markers.MarkerStyle(marker='o', fillstyle='full'), facecolor='black', s=.1, zorder=3)
	...
	def show_complex(filt_plot, i):
		...
		if color_scheme == 'none':
		...
		elif color_scheme == 'highlight new':
			...
			print "WARNING: 'color_scheme' not recognized. Using color_scheme='none'."
